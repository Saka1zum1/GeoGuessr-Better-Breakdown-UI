...
const polylineRoundMap = new WeakMap();


const SELECTORS = {
    guessMarker: '[class^="map-pin_mapPin__"]:not([data-qa="correct-location-marker"])',
    ...
    challengeEnd: "[class*=coordinate-results_clickableColumn]",
    duelEndMap: "[class^='game-finished-2_breakdownHeader']",
...
googleMapsPromise.then(() => runAsClient(() => {
    ...
    if (!google?.maps?.Polyline) {
        throw new Error('google.maps.Polyline not found');
    }

    OldPolyline = google.maps.Polyline;

    function WrappedPolyline(...args) {
        const polyline = new OldPolyline(...args);

        try {
            polyline._isPagePolyline = true;

            if (!window.allPolylinesRef) {
                window.allPolylinesRef = [];
            }
            window.allPolylinesRef.push(polyline);
        } catch (e) {
            console.warn('[Polyline hook][onCreate error]', e);
        }

        return polyline;
    }

    WrappedPolyline.prototype = OldPolyline.prototype;
    Object.setPrototypeOf(WrappedPolyline, OldPolyline);

    google.maps.Polyline = WrappedPolyline;

    return {
        restore() {
            google.maps.Polyline = OldPolyline;
        }
    };
}))
...
async function gameLoop() {
    ...
    const challengeEndEl = domCache.queryOne(SELECTORS.challengeEnd);
    ...
    const ischallengeEnd = !!challengeEndEl;
    ...
    if (!isRoundEnd && !isGameEnd && !isDuelEnd && !ischallengeEnd) {
        removePeekMarker();
        stopPathPlayback();
        clearMovementPaths()
        toggleMapType(true)
        if (isCoverageLayer) toggleCoverageLayer("off");
        if(unsafeWindow.allPolylinesRef && unsafeWindow.allPolylinesRef?.length>0) unsafeWindow.allPolylinesRef=[]
        return
    }

    if ((!token || !round) && !isDuelEnd && !ischallengeEnd && !isReplay) return;

    if (isRoundEnd || isGameEnd || isDuelEnd || ischallengeEnd) {
        if (reactionBtn) reactionBtn.remove()
        if (isRoundEnd && currentMovementRound && currentMovementPath.length > 2) {
...
function attachClickListener(map) {
    map.addListener("click", async (e) => {
        ...
        if ((domCache.queryOne(SELECTORS.roundEnd) ||
             ...
             domCache.queryOne(SELECTORS.challengeEnd) ||
             (domCache.queryOne(SELECTORS.duelEnd))) && !domCache.queryOne(SELECTORS.duelEndMap)) {
...
function setMapControls(container) {
    if(!container) return
    let duelEndMiniMap = domCache.queryOne(SELECTORS.duelEndMap)
    ...
    if ((coverageLayerControl && pathControl && mapTypeControl) || duelEndMiniMap) return
...
function focusPath() {
    ...
    const visibleMarkers = Array.from(answerMarkers).filter(el => {
    ...
    let index;
    const isChallenge = domCache.queryOne(SELECTORS.challengeEnd)
    if (isChallenge) {
        const roundColoums = domCache.queryAll("[class*='coordinate-results_clickableColumn']")
        const selectedColoum = domCache.queryOne("[class*='coordinate-results_selectedColumn']")
        index = Array.prototype.indexOf.call(roundColoums, selectedColoum) + 1;
    }

    else index = Array.prototype.indexOf.call(answerMarkers, visibleMarkers[0]) + 1;
    const round =answerMarkers.length == visibleMarkers.length && !isChallenge ? getCurrentRound() : index;
    ...
    guessMap.fitBounds(bounds, { padding: 100 });
    if (pathControl) {
...
function playPathAnimation(path) {
    ...
    const polyline = new OldPolyline({
        path: [],
        geodesic: true,
    ...
    const visibleGuessMarkers = Array.from(guessMarkerElements).filter(el => {
        return getComputedStyle(el).display !== 'none';
    });
    if (visibleGuessMarkers.length > 0) {
        ...
        const originalMarker = visibleGuessMarkers[0];
...
function addCreditToPage(container, duelMap) {
    ...
    element.innerHTML = `
		<div class="peek-credit-title">GeoGuessr Better Breakdown</div>
    ...
    `,
        ...
        didOpen: () => {
            ...
            toggleSVBtn.addEventListener('click', async () => {
                ...
                const metaData = await getLOCATION(panoId);
...
function clearMovementPaths() {
    ...
    if (overlay) {
        if (Array.isArray(overlay)) {
            overlay.forEach(item => {overlay.forEach(item => item.polyline?.setMap(null));});
        } else if (overlay.polyline) overlay.polyline.setMap(null);
    }
...
function recordMovementPoint(panorama) {
    ...
    if(!currentMovementRound)currentMovementRound=round

    if (currentMovementRound !== round) {
        currentMovementPath = [newPoint];
        currentMovementRound = round;
        return;
    }
...
function handleRoundElementClick(num) {
    ...
    if (unsafeWindow.allPolylinesRef) {
        unsafeWindow.allPolylinesRef.forEach((polyline, index) => {

            try {
                if (index === num || showAll) {
                    polyline.setMap(guessMap);
                }
                else polyline.setMap(null);
            } catch (error) {
                console.warn(`[Polyline ${index}] Error:`, error);
            }
        });
    }
...
function openNativeStreetView(pano) {
    ...
    const mapContainer = domCache.queryOne(SELECTORS.resultMap) || domCache.queryOne(SELECTORS.duelMap);
...
function showMapList() {
    ...
    // Append to fullscreen element if in fullscreen, otherwise append to body
...
function attachMarkerClickListener(marker, pano) {
    if (domCache.queryOne(SELECTORS.duelEndMap)) return
